...
// radium::OuterAppBundle(), which should be the only caller.
NSBundle* OuterAppBundleInternal() {
  @autoreleasepool {
    ...
    // steps to C.app.
    base::FilePath framework_path = radium::GetFrameworkBundlePath();
...
std::string ProductDirName() {
  ...
  static const char* product_dir_name =
      ProductDirNameForBundle(radium::OuterAppBundle());
...
base::FilePath GetFrameworkBundlePath() {
  ...
  if (base::apple::IsBackgroundOnlyProcess()) {
    ...
    path = path.Append("Frameworks")
               ...
               .Append("Versions")
               .Append(kRadiumVersion);
  }
  DCHECK_EQ(path.BaseName().value(), kRadiumVersion);
