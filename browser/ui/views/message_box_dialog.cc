...
#include "radium/browser/ui/views/message_box_dialog.h"
...
#if BUILDFLAG(IS_MAC)
#include "radium/browser/ui/cocoa/simple_message_box_cocoa.h"
#endif

namespace {

#if BUILDFLAG(IS_WIN)
UINT GetMessageBoxFlagsFromType(radium::MessageBoxType type) {
  UINT flags = MB_SETFOREGROUND;
  switch (type) {
    case radium::MESSAGE_BOX_TYPE_WARNING:
      return flags | MB_OK | MB_ICONWARNING;
    case radium::MESSAGE_BOX_TYPE_QUESTION:
...
radium::MessageBoxResult ShowSync(gfx::NativeWindow parent,
  ...
  if (g_message_box_is_showing_sync) {
    return radium::MESSAGE_BOX_RESULT_NO;
  }

  base::AutoReset<bool> is_showing(&g_message_box_is_showing_sync, true);
  radium::MessageBoxResult result = radium::MESSAGE_BOX_RESULT_NO;
  base::RunLoop run_loop(base::RunLoop::Type::kNestableTasksAllowed);
  MessageBoxDialog::Show(
      parent, title, message, type, yes_text, no_text, checkbox_text,
      base::BindOnce(
          [](base::RunLoop* run_loop, radium::MessageBoxResult* out_result,
             radium::MessageBoxResult messagebox_result) {
...
void ShowNativeMessageBox(gfx::NativeWindow parent,
  ...
  std::move(callback).Run(
      radium::ShowMessageBoxCocoa(message, type, checkbox_text));
#else
  NOTREACHED();
#endif
}
...
// static
radium::MessageBoxResult MessageBoxDialog::Show(
  ...
  if (radium::internal::g_should_skip_message_box_for_test) {
    std::move(callback).Run(radium::MESSAGE_BOX_RESULT_YES);
    return radium::MESSAGE_BOX_RESULT_DEFERRED;
  }
  ...
  if (!CanUseViewsMessageBox()) {
    ...
    std::move(callback).Run(radium::MESSAGE_BOX_RESULT_NO);
    return radium::MESSAGE_BOX_RESULT_DEFERRED;
  }
...
void MessageBoxDialog::OnWidgetDestroying(views::Widget* widget) {
  ...
  // result.
  if (result_callback_) {
    Done(radium::MESSAGE_BOX_RESULT_NO);
  }
}
...
MessageBoxDialog::MessageBoxDialog(std::u16string_view title,
  ...
  SetButtons(type_ == radium::MESSAGE_BOX_TYPE_QUESTION
  ...
  if (ok_text.empty()) {
    ok_text =
        type_ == radium::MESSAGE_BOX_TYPE_QUESTION
  ...
  if (type_ == radium::MESSAGE_BOX_TYPE_QUESTION) {
...
void MessageBoxDialog::Done(radium::MessageBoxResult result) {
...
MessageBoxResult ShowWarningMessageBox(gfx::NativeWindow parent,
  ...
  return MessageBoxDialog::Show(
      parent, title, message, radium::MESSAGE_BOX_TYPE_WARNING,
...
void ShowWarningMessageBoxWithCheckbox(
  ...
  MessageBoxDialog::Show(parent, title, message,
                         radium::MESSAGE_BOX_TYPE_WARNING, std::u16string(),
...
MessageBoxResult ShowQuestionMessageBoxSync(gfx::NativeWindow parent,
                                            ...
                                            const std::u16string& message) {
  return MessageBoxDialog::Show(
      parent, title, message, radium::MESSAGE_BOX_TYPE_QUESTION,
      std::u16string(), std::u16string(), std::u16string());
}
...
void ShowQuestionMessageBox(
  ...
  MessageBoxDialog::Show(parent, title, message,
                         radium::MESSAGE_BOX_TYPE_QUESTION, std::u16string(),
...
MessageBoxResult ShowMessageBoxWithButtonText(gfx::NativeWindow parent,
  ...
  return MessageBoxDialog::Show(parent, title, message,
                                radium::MESSAGE_BOX_TYPE_QUESTION, yes_text,
