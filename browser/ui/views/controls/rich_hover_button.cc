...
#include "radium/browser/ui/views/controls/rich_hover_button.h"
...
#include "radium/browser/ui/views/accessibility/non_accessible_image_view.h"
...
RichHoverButton::RichHoverButton() {
  ...
  views::Builder<RichHoverButton>(this)
      .SetBorder(
          views::CreateEmptyBorder(RadiumLayoutProvider::Get()->GetInsetsMetric(
              RadiumInsetsMetric::INSETS_PAGE_INFO_HOVER_BUTTON)))
...
void RichHoverButton::RecreateLayout() {
  const int icon_label_spacing = RadiumLayoutProvider::Get()->GetDistanceMetric(
