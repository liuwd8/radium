...
+ (NSApplication*)sharedApplication {
  ...
  CHECK([NSApp isKindOfClass:self])
      << "NSApp must be of type " << [[self className] UTF8String] << ", not "
...
- (BOOL)sendAction:(SEL)anAction to:(id)aTarget from:(id)sender {
  ...
  std::string value = base::StringPrintf(
      "%s tag %ld sending %s to %p", [[sender className] UTF8String],
      static_cast<long>(tag), [actionString UTF8String], aTarget);
...
- (id)accessibilityFocusedUIElement {
  if (id forced_focus =
          ui::AccessibilityFocusOverrider::GetFocusedUIElement()) {
    return forced_focus;
  }
