...
HWND FindRunningRadiumWindow(const base::FilePath& user_data_dir) {
  TRACE_EVENT0("startup", "FindRunningRadiumWindow");
  return base::win::MessageWindow::FindWindow(user_data_dir.value());
}

NotifyRadiumResult AttemptToNotifyRunningRadium(HWND remote_window) {
  TRACE_EVENT0("startup", "AttemptToNotifyRunningRadium");
  ...
  if (!thread_id || !process_id) {
    TRACE_EVENT0(
        "startup",
        "AttemptToNotifyRunningRadium:GetWindowThreadProcessId failed");
    return NotifyRadiumResult::NOTIFY_FAILED;
  }

  base::FilePath cur_dir;
  if (!base::GetCurrentDirectory(&cur_dir)) {
    TRACE_EVENT_INSTANT(
        "startup", "AttemptToNotifyRunningRadium:GetCurrentDirectory failed");
    return NotifyRadiumResult::NOTIFY_FAILED;
  }
  ...
  DWORD_PTR result = 0;
  {
    TRACE_EVENT0("startup", "AttemptToNotifyRunningRadium:SendMessage");
    if (::SendMessageTimeout(remote_window, WM_COPYDATA, NULL,
      ...
      return result ? NotifyRadiumResult::NOTIFY_SUCCESS
                    : NotifyRadiumResult::NOTIFY_FAILED;
    }
  }
  ...
  if (!timed_out) {
    TRACE_EVENT_INSTANT("startup",
                        "AttemptToNotifyRunningRadium:Error SendFailed");
    return NotifyRadiumResult::NOTIFY_FAILED;
  }
  ...
  if (!::IsWindow(remote_window)) {
    TRACE_EVENT_INSTANT("startup",
                        "AttemptToNotifyRunningRadium:Error RemoteDied");
    return NotifyRadiumResult::NOTIFY_FAILED;
  }

  // If the window couldn't be notified but still exists, assume it is hung.
  TRACE_EVENT_INSTANT("startup",
                      "AttemptToNotifyRunningRadium:Error RemoteHung");
  return NotifyRadiumResult::NOTIFY_WINDOW_HUNG;
}
