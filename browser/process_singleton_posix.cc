...
// enough.
constexpr int kTimeoutInSeconds = 20;
...
constexpr char kTokenDelimiter = '\0';
constexpr int kMaxMessageLength = 32 * 1024;
constexpr int kMaxACKMessageLength = kShutdownToken.size() - 1;
...
bool WriteToSocket(int fd, std::string_view message) {
  ...
  do {
    ssize_t rv = HANDLE_EINTR(write(fd, message.substr(bytes_written).data(),
...
bool IsChromeProcess(pid_t pid) {
  ...
  return (!other_chrome_path.empty() &&
          other_chrome_path.BaseName() ==
              base::FilePath(radium::kBrowserProcessExecutableName));
}
...
void ProcessSingleton::LinuxWatcher::SocketReader::
  ...
  constexpr size_t kMinMessageLength = kStartToken.size() + 4;
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcessWithTimeout(
  ...
  buf[len] = '\0';
  if (kShutdownToken == buf) {
    // The other process is shutting down, it's safe to start a new process.
    internal::SendRemoteProcessInteractionResultHistogram(
        REMOTE_PROCESS_SHUTTING_DOWN);
    return PROCESS_NONE;
  } else if (kACKToken == buf) {
