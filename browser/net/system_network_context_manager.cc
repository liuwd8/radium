...
#include "radium/browser/net/convert_explicitly_allowed_network_ports_pref.h"
#include "radium/browser/net/default_dns_over_https_config_source.h"
#include "radium/browser/net/radium_mojo_proxy_resolver_factory.h"
...
NetworkSandboxState IsNetworkSandboxEnabledInternal() {
  ...
  auto* local_state = BrowserProcess::Get()->local_state();
#endif
...
void SystemNetworkContextManager::OnNetworkServiceCreated(
  ...
  // // requires the SystemNetworkContextManager to be ready. It does not need
  // to
  // // be recreated every time the network service is restarted (and should
  // not,
  // // since it expects to outlive the NetworkTimeTracker).
  // // BrowserProcess::Get() can be null in some tests.
  // if (base::FeatureList::IsEnabled(features::kCertVerificationNetworkTime) &&
  //     !cert_verifier_time_updater_ && BrowserProcess::Get()) {
  //   cert_verifier_time_updater_ =
  //       std::make_unique<CertVerifierServiceTimeUpdater>(
  //           BrowserProcess::Get()->network_time_tracker());
  // }
}
  ...
  if (!command_line.HasSwitch(switches::kWinHttpProxyResolver)) {
    if (command_line.HasSwitch(switches::kSingleProcess)) {
      ...
      network_context_params->proxy_resolver_factory =
          RadiumMojoProxyResolverFactory::CreateWithSelfOwnedReceiver();
...
SystemNetworkContextManager::GetHttpAuthStaticParamsForTesting() {
  return CreateHttpAuthStaticParams(BrowserProcess::Get()->local_state());
}

network::mojom::HttpAuthDynamicParamsPtr
SystemNetworkContextManager::GetHttpAuthDynamicParamsForTesting() {
  return CreateHttpAuthDynamicParams(BrowserProcess::Get()->local_state());
}
...
bool SystemNetworkContextManager::IsCertificateTransparencyEnabled() {
  ...
  // //   - on an opt-in basis for other builds and embedders, controlled with
  // the
