...
// #include
// "radium/browser/enterprise/client_certificates/certificate_provisioning_service_factory.h"
...
void UpdateCookieSettings(Profile* profile, ContentSettingsType type) {
  ...
  if (type == ContentSettingsType::FEDERATED_IDENTITY_SHARING) {
    ...
    // // have propagated before it calls its callback. However, the syncing
    // that
...
void ProfileNetworkContextService::ConfigureNetworkContextParams(
  ...
  if ((!in_memory && !profile_->IsOffTheRecord())) {
    ...
    radium::GetUserCacheDirectory(GetPartitionPath(relative_partition_path),
                                  &base_cache_path);
    base::FilePath media_cache_path =
        base_cache_path.Append(radium::kMediaCacheDirname);
...
void ProfileNetworkContextService::DisableQuicIfNotAllowed() {
  ...
  BrowserProcess::Get()->system_network_context_manager()->DisableQuic();
}
...
void ProfileNetworkContextService::ConfigureNetworkContextParamsInternal(
  ...
  if (!in_memory) {
    ...
    base::FilePath base_cache_path;
    radium::GetUserCacheDirectory(path, &base_cache_path);
    ...
    network_context_params->file_paths->http_cache_directory =
        base_cache_path.Append(radium::kCacheDirname);
    network_context_params->file_paths->data_directory =
        path.Append(radium::kNetworkDataDirname);
    network_context_params->file_paths->unsandboxed_data_path = path;
    network_context_params->file_paths->trigger_migration =
        base::FeatureList::IsEnabled(kTriggerNetworkDataMigration);
    ...
    network_context_params->file_paths->trust_token_database_name =
        base::FilePath(radium::kTrustTokenFilename);

#if BUILDFLAG(ENABLE_REPORTING)
    network_context_params->file_paths->reporting_and_nel_store_database_name =
        base::FilePath(radium::kReportingAndNelStoreFilename);
  ...
  //   if
  //   (base::FeatureList::IsEnabled(features::kEnableCertManagementUIV2Write))
  //   {
  ...
  network_context_params->reset_http_cache_backend =
      GetHttpCacheBackendResetParam(BrowserProcess::Get()->local_state());
