...
#include "radium/browser/process_singleton_internal.h"
#include "radium/browser/ui/simple_message_box.h"
#include "radium/browser/win/radium_process_finder.h"
...
bool DisplayShouldKillMessageBox() {
  ...
  return radium::ShowQuestionMessageBoxSync(
...
void TerminateProcessWithHistograms(const base::Process& process,
  ...
  if (result) {
    ...
    UMA_HISTOGRAM_TIMES("Radium.ProcessSingleton.TerminateProcessTime",
                        base::TimeTicks::Now() - start_time);
    base::UmaHistogramSparse(
        "Radium.ProcessSingleton.TerminationWaitErrorCode.Windows", wait_error);
  } else {
  ...
  base::UmaHistogramSparse(
      "Radium.ProcessSingleton.TerminateProcessErrorCode.Windows",
      terminate_error);
}
...
bool ProcessSingleton::EscapeVirtualization(
  ...
  if (::GetModuleHandle(L"sftldr_wow64.dll") ||
    ...
    for (int tries = 200; tries; --tries) {
      hwnd = FindRunningRadiumWindow(user_data_dir);
      if (hwnd) {
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcess() {
  ...
  switch (AttemptToNotifyRunningRadium(remote_window_)) {
    case NotifyRadiumResult::NOTIFY_SUCCESS:
      return PROCESS_NOTIFIED;
    case NotifyRadiumResult::NOTIFY_FAILED:
      remote_window_ = NULL;
      internal::SendRemoteProcessInteractionResultHistogram(
          RUNNING_PROCESS_NOTIFY_ERROR);
      return PROCESS_NONE;
    case NotifyRadiumResult::NOTIFY_WINDOW_HUNG:
...
ProcessSingleton::NotifyResult ProcessSingleton::NotifyOtherProcessOrCreate() {
  ...
  for (int i = 0; i < 2; ++i) {
    if (Create()) {
      DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES(
          "Radium.ProcessSingleton.TimeToCreate",
    ...
    if (result == PROCESS_NOTIFIED || result == LOCK_ERROR) {
      if (result == PROCESS_NOTIFIED) {
        DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES(
            "Radium.ProcessSingleton.TimeToNotify",
            base::TimeTicks::Now() - begin_ticks);
      } else {
        DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES(
            "Radium.ProcessSingleton.TimeToFailure",
            base::TimeTicks::Now() - begin_ticks);
      }
  ...
  DEPRECATED_UMA_HISTOGRAM_MEDIUM_TIMES("Radium.ProcessSingleton.TimeToFailure",
                                        base::TimeTicks::Now() - begin_ticks);
  return PROFILE_IN_USE;
}

// Look for a Radium instance that uses the same profile directory. If there
// isn't one, create a message window with its title set to the profile
// directory path.
bool ProcessSingleton::Create() {
  TRACE_EVENT0("startup", "ProcessSingleton::Create");

  static const wchar_t kMutexName[] = L"Local\\RadiumProcessSingletonStartup!";

  remote_window_ = FindRunningRadiumWindow(user_data_dir_);
  if (!remote_window_ && !EscapeVirtualization(user_data_dir_)) {
    ...
    if (!remote_window_) {
      // We have to make sure there is no Radium instance running on another
      ...
      if (lock_file_ != INVALID_HANDLE_VALUE) {
        ...
        // other Radium instances can decide if they should forward to us.
