...
def reorder_imports(input_dir, output_dir, architecture):
    """Swap radium_elf.dll to be the first import of radium.exe.
    ...
    input_image = os.path.join(input_dir, 'radium.exe')
    output_image = os.path.join(output_dir, 'radium.exe')
    ...
    for i, peimport in enumerate(pe.DIRECTORY_ENTRY_IMPORT):
        if peimport.dll.lower() == b'radium_elf.dll':
            assert not found_elf, 'only one radium_elf.dll import expected'
            found_elf = True
            if i > 0:
    ...
    assert found_elf, 'radium_elf.dll import not found'

    pe.write(filename=output_image)

    for fname in glob.iglob(os.path.join(input_dir, 'radium.exe.*')):
...
def main(argv):
    ...
    parser.add_option('-i',
                      '--input',
                      help='reorder radium.exe in DIR',
                      metavar='DIR')
    parser.add_option('-o',
                      '--output',
                      help='write new radium.exe to DIR',
                      metavar='DIR')
    parser.add_option('-a',
                      '--arch',
                      help='architecture of build (optional)',
                      default='ia32')
