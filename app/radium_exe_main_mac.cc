...
typedef int (*RadiumMainPtr)(int, char**);
...
__attribute__((visibility("default"))) int main(int argc, char* argv[]) {
  ...
  const char rel_path[] = "../Frameworks/" PRODUCT_FULLNAME_STRING
                          " Framework.framework/Versions/" RADIUM_VERSION_STRING
  ...
  const RadiumMainPtr radium_main =
      reinterpret_cast<RadiumMainPtr>(dlsym(library, "RadiumMain"));
  if (!radium_main) {
    FatalError("dlsym RadiumMain: %s.", dlerror());
  }
  rv = radium_main(argc, argv);
