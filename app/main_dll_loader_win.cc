...
namespace {
constexpr wchar_t kRadiumDll[] = L"radium.dll";
...
typedef void (*RelaunchRadiumWithNewCommandLineIfNeededFunc)();
...
base::FilePath GetModulePath(std::wstring_view module_name) {
  ...
  const base::FilePath module_path =
      exe_dir.AppendASCII(radium::kRadiumVersion).Append(module_name);
...
HMODULE Load(base::FilePath* module,
             ...
             base::TimeTicks& preread_begin_ticks,
             base::TimeTicks& preread_end_ticks) {
  *module = GetModulePath(kRadiumDll);
  if (module->empty()) {
    PLOG(ERROR) << "Cannot find module " << kRadiumDll;
    return nullptr;
  }
  HMODULE dll = LoadModuleWithDirectory(*module, cmd_line, is_browser,
                                        preread_begin_ticks, preread_end_ticks);
  if (!dll) {
    PLOG(ERROR) << "Failed to load Radium DLL from " << module->value();
  }
  return dll;
}
...
int MainDllLoader::Launch(HINSTANCE instance,
  ...
  DLL_MAIN radium_main =
      reinterpret_cast<DLL_MAIN>(::GetProcAddress(dll_, "RadiumMain"));
  int rc = radium_main(instance, &sandbox_info,
...
class RadiumDllLoader : public MainDllLoader {
 protected:
...
void RadiumDllLoader::OnBeforeLaunch(const std::string& process_type,
